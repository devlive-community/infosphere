  ...
  return (
    <div className="mb-4">
      <Input
        ...
        trailing={keyword ? (
          <button type="button" onClick={() => setKeyword('')} aria-label="清空搜索" className="text-slate-400 transition-colors hover:text-slate-600">
            <CloseIcon className="h-4 w-4" />
          </button>
        ) : undefined}
        placeholder="在本书内搜索章节标题或内容…"
        maxLength={100}
      />
      ...
      {q && (
        <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white">
          {loading && hits.length === 0 ? (
            <Loading className="py-6" label="正在搜索本书章节…" />
          ) : searched && hits.length === 0 ? (
            <p className="py-6 text-center text-sm text-slate-400">本书内未找到与「{q}」相关的章节</p>
          ) : (
            <>
              <p className="border-b border-slate-100 px-4 py-2.5 text-xs text-slate-400">本书内命中 {total} 个章节</p>
