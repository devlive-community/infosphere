...
export default function BookCopyDialog({ book, tree, open, onClose }: { book: Book; tree?: Document[]; open: boolean; onClose: () => void }) {
  ...
  }, [open, book.id, book.title, tree]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function submit() {
    ...
    try {
      ...
      showToast({ title: t('copyDialog.failed'), message: (e as Error).message, tone: 'error' })
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <Modal open={open} onClose={onClose} title={t('copyDialog.title')} className="max-w-xl"
      footer={<>
      ...
      <div className="space-y-4">
        ...
        {mode === 'custom' && (
          items.length === 0 ? (
            <p className="rounded-lg border border-dashed border-slate-200 py-6 text-center text-sm text-slate-400">{t('copyDialog.emptyKeep')}</p>
          ) : (
                ...
                return (
                  <li key={it.id} draggable
                    ...
                    <button type="button" onClick={() => removeSubtree(i)}
                      aria-label={t('copyDialog.removeChapter')} className="shrink-0 rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-rose-500">
