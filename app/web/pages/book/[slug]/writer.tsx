...
export default function Writer() {
  ...
  function deselect() {
    if (!current) return
    resetForm()
    router.push(`/book/${encodeURIComponent(bookSlug)}/writer`, undefined, { shallow: true })
  }
  ...
  function selectDoc(slug: string) {
    router.push(`/book/${encodeURIComponent(bookSlug)}/writer?doc=${encodeURIComponent(slug)}`, undefined, { shallow: true })
  }
  ...
  async function removeDoc(doc: Document) {
    ...
    try {
      ...
      await loadTree(book)
      if (current?.id === doc.id) { resetForm(); router.push(`/book/${encodeURIComponent(bookSlug)}/writer`, undefined, { shallow: true }) }
    } catch (e) { alert((e as Error).message) }
  }
