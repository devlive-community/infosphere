    ...
    <Container>
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-6">
          ...
          {canManage && (
            <div className="mt-4 flex gap-2">
              <ButtonLink href={`/book/${encodeURIComponent(book.slug)}/writer`}>+ 新建章节</ButtonLink>
              <ButtonLink href={`/book/${encodeURIComponent(book.slug)}/settings`} variant="outline">书籍设置</ButtonLink>
            </div>
          )}
        </div>
      </div>
      ...
      <aside className="rounded-xl border border-slate-200 bg-white shadow-sm h-fit p-4">
        ...
        <DocTree
          items={tree}
          itemRender={(doc) => (
            <Link href={`/book/${encodeURIComponent(book.slug)}/${doc.slug}/reader`} className="flex-1 truncate hover:text-primary-600">
              {chapterPrefix}{doc.title}
            </Link>
          )}
        />
        {canManage && (
          <ButtonLink href={`/book/${encodeURIComponent(book.slug)}/writer`} variant="outline" className="mt-3 w-full">管理章节</ButtonLink>
        )}
      </aside>
    </div>
    </Container>
