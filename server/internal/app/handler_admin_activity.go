...
func (a *App) AdminStats(c *gin.Context) {
	...
	stats := gin.H{"user_count": userCount, "book_count": bookCount, "document_count": docCount, "total_views": views}
	plugincore.CollectStats(a, false, stats) // 插件补充的统计（如标签插件的 tag_count）
	ok(c, stats)
}
